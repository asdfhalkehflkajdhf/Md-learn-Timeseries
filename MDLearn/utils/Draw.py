...
def image_to_show(W):
    ...
    image = Image.fromarray(
        _tile_raster_images(
            X=W.T,
            img_shape=(int(math.sqrt(W.shape[0])),
                       int(math.sqrt(W.shape[0]))),
            tile_shape=(int(math.sqrt(W.shape[1])),
                        int(math.sqrt(W.shape[1]))),
            tile_spacing=(1, 1)
        )
    )
    plt.imshow(image)
    ...
    if isinstance(X, tuple):
        ...
        for i in range(4):
            if X[i] is None:
                ...
                # in the output
                out_array[:, :, i] = _tile_raster_images(
