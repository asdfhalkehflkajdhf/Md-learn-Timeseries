...
class RBM(object):
    ...
    def get_errs(self, X):
        ...
        h_sample = self.sample_prob(tf.nn.sigmoid(tf.matmul(input_X, _w) + _hb))
        v_sample = self.sample_prob(tf.nn.sigmoid(tf.matmul(h_sample, tf.transpose(_w)) + _vb))
        err = X - v_sample
        err_mean = tf.reduce_mean(err * err)
        with tf.Session() as sess:
            sess.run(tf.initialize_all_variables())
            return sess.run(err_mean)
