...
class Dropout(object):
    ...
    def train(self, epochs=5000, dropout=True, p_dropout=0.5, rng=None):
        ...
        for epoch in range(epochs):
            ...
            bp_err = self.log_layer.train(input=layer_input)
            ...
            for i in reversed(range(0, self.n_layers)):
                ...
                if dropout == True:
                    ...
                    bp_err = self.hidden_layers[i].backward(inputs=prev_layer.x, bp_err=bp_err, W=prev_layer.W, dropout=True, mask=dropout_masks[i])
                else:
                    bp_err = self.hidden_layers[i].backward(inputs=prev_layer.x, bp_err=bp_err, W=prev_layer.W)
