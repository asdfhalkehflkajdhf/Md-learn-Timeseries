...
在CD算法一开始，可见单元的状态就被设置为一个训练样本，并用上面的几个条件概率来对隐藏节点的每个单元都从{0,1}中抽取到相应的值，然后再利用 来对可视节点的每个单元都从{0,1}中抽取相应的值，这样就得到了v1了，一般v1就够了，就可以拿来估算梯度了。
        ...
        #Sr
        L_Norm_W = numpy.dot(self.input.T, nh_samples*(1-nh_samples) )
        L_Norm_b = numpy.mean( (nh_samples*(1-nh_samples)), axis=0)
        print(L_Norm_W.mean(), L_Norm_b.mean() , self.__delta_w.mean(), self.__delta_hb.mean())
        self.W -= L_Norm_W
        self.hbias -= L_Norm_b
        # cost = self.get_reconstruction_cross_entropy()
        # return cost
...
def Test_rbmMNIST():
    ...
    errs = rbm.train(input=trX,lr=0.1,epochs=1, batch_size=1000, show=True, show_epochs=1)
    import matplotlib.pyplot as plt
    import matplotlib
    matplotlib.rcParams['font.sans-serif'] = [u'SimHei']
    matplotlib.rcParams['axes.unicode_minus'] = False

    from MDLearn.utils.Draw import image_to_show
